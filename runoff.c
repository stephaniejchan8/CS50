...
bool vote(int voter, int rank, string name)
{
    for (int i = 0; i < candidate_count; i++)
    {
        if (strcmp(candidates[i].name, name) == 0)
        {
            ...
            // Invalid vote if voter votes for same candidate multiple times
...
bool print_winner(void)
{
    ...
    int minimum_win = voter_count / 2;
    if (highest_score > minimum_win)
    {
        for (int i = 0; i < candidate_count; i++)
        {
            if (candidates[i].votes == highest_score)
...
int find_min(void)
{
    ...
    for (int i = 0; i < candidate_count; i++)
    {
        if (candidates[i].votes < lowest_score && !candidates[i].eliminated)
        {
